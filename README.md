...
```bash
app2$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
app2$ sam local start-api
app2$ curl http://localhost:3000/
```
...
```bash
app2$ sam logs -n HelloWorldFunction --stack-name "app2" --tail
```
...
```bash
app2$ pip install pytest pytest-mock --user
app2$ python -m pytest tests/ -v
```
